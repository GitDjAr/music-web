  ...
  <div class="myclass  box-border flex select-none overflow-hidden h-full w-full relative" v-bind="$attrs">
    ...
    <div class="w-4/5 " ref="refBox">
      <div :class="`text-left  pl-5  pb-4  transition-all w-full flex flex-col justify-between  h-52 `">
        ...
        <p>创建日期: {{ formatformat(Albums.createTime, 'yyyy-MM-dd') }}</p>
      ...
      <div class="hybull  pl-5 bg-white bg-opacity-90 backdrop-filter backdrop-blur-sm flex flex-col overflow-y-auto"
        ...
        <div v-for="item in AlbumsList" :key="item.id" @click="play(item)"
          ...
          <div class="flex-1 mx-2 text-left inline-block truncate"> {{ item.name }}</div>
...
const AlbumsList = ref<Array<object>>([])
...
function play(item) {
  store.dispatch("ToggleSong", { id: item.id, playListId: id, list: AlbumsList.value });
}
</script>
